...
pub fn create_face(font_name: &str) -> BoxResult<freetype::Face> {
    ...
    let font = fc
        .find(font_name, None)
        .ok_or(Box::new(SimpleError::new("could not find font")))?;
...
pub fn create_glyph<C: Connection>(
    ...
    for y in 0..glyphinfo.height {
        output_bitmap
            [(y * stride) as usize..((y + 1) * stride - (stride - glyphinfo.width)) as usize]
            .copy_from_slice(
                &input_bitmap[(y * glyphinfo.width) as usize..((y + 1) * glyphinfo.width) as usize],
            );
    }
...
pub fn render_glyph<C: Connection>(conn: &C) -> BoxResult<()> {
    Ok(())
}
